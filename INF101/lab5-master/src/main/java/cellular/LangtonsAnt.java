...
public class LangtonsAnt implements ICellAutomaton {
	...
	public void initializeCells() {
		...
		int midX = this.currentGeneration.rows() / 2,
				midY = this.currentGeneration.cols() / 2;
		CellPosition midLoc = new CellPosition(midX, midY);
	...
	public void step() {
		...
		if (nextAnt.getRow() >= currentGeneration.rows()) {
			nextAnt.setRow(currentGeneration.rows() - 3);
		}
		if (nextAnt.getCol() >= currentGeneration.cols()) {
			nextAnt.setCol(currentGeneration.cols() - 3);
		}
		if (nextAnt.getRow() < 0) {
			nextAnt.setRow(2);
		}
